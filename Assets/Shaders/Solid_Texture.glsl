/*
This code creates basis raytracing image with solid texture and area light shadow
scene consists of a one plane and one sphere:
iChannel0: Texture Image (the same image are applied to both shapes)
*/
...
//if(st.x<0.0) st.x=st.x+1.0;
//if(st.y<0.0) st.y=st.y+1.0; 
//if(st.z<0.0) st.z=st.z+1.0; 
...
vec2  p2(vec2 st)   
{
st.x=st.x-float(int(st.x));
st.y=st.y-float(int(st.y));
if(st.x<0.0) st.x=st.x+1.0;
if(st.y<0.0) st.y=st.y+1.0;    
st.x=abs(2.0*(st.x-0.5));
st.y=abs(2.0*(st.y-0.5));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    */
    ...
    float s2=iMouse.x*iResolution.y/iResolution.x;
    */
    float s0=300.;
    float s1=300.;
    float s2=300.;
    ...
    mint=t;
    vec3 uvw;
    uvw.x=dot(N0,(P_H-P_C))/s0; 
    uvw.y=dot(N1,(P_H-P_C))/s1; 
    uvw.z=dot(N2,(P_H-P_C))/s2; 
    uvw=p2_3D(uvw);
    BG = texture(iChannel0, uvw.xy);
    BG1 = texture(iChannel1, uvw.xy);
    dif = BG*(1.0-uvw.z)+BG1*uvw.z;
    ambient = (dif+ambient0)/4.0; 
  ...
  /*
    ...
    N=N_PL;
    vec3 temp;
    temp.x=dot(N0,(P_H-P_C))/s0; 
    temp.y=dot(N1,(P_H-P_C))/s1; 
    temp.z=dot(N2,(P_H-P_C))/s2; 
    uv_tex.y=temp.x;
    uv_tex.x=temp.y;    
    BG = texture(iChannel0, uv_tex);
    BG1 = texture(iChannel1, uv_tex);
    float w=temp.z-float(int(temp.z));
    w=abs(2.0*(w-0.5));
    //float w=temp.z;
    dif = BG*(1.0-w)+BG1*w;
    ambient = (dif+ambient0)/4.0; 
