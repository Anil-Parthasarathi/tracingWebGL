/*
This code creates basis raytracing image with texture mapping and area light shadow
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    */
    ...
    N=(P_H-P_C)/R;
    vec3 temp;
    temp.x=dot(N0,(P_H-P_C))/R; 
    temp.y=dot(N1,(P_H-P_C))/R; 
    temp.z=dot(N2,(P_H-P_C))/R;
    
    uv_tex.y=acos(temp.z)/pi;
    
    //uv_tex.x=acos(temp.y/sqrt(1.0-temp.z*temp.z))/(2.0*pi);
    //uv_tex.x=acos(temp.y/sin(uv_tex.y*pi))/(2.0*pi);
    //if(temp.x<=0.0) uv_tex.x = 1.0-uv_tex.x;
    
    uv_tex.x=atan(temp.x,temp.y)/(2.0*pi);  
    uv_tex=1.0*uv_tex;
    uv_tex=p2(uv_tex);
    BG = texture(iChannel0, uv_tex);
    ambient = (BG+ambient0)/4.0; 
    dif = BG;
